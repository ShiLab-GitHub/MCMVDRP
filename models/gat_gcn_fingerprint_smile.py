...
class GAT_GCN_Fingerprint_SMILES(torch.nn.Module):
    def __init__(self, n_output=1, num_features_xd=78, num_features_xt=25,
                 n_filters=32, embed_dim=128, output_dim=128, dropout=0.2):

        super(GAT_GCN_Fingerprint_SMILES, self).__init__()

        #fingerprint features

        self.embedding_xf = nn.Embedding(num_features_xt + 1, embed_dim * 2)

        self.conv_xf_1 = nn.Conv1d(in_channels=167, out_channels=n_filters, kernel_size=8)
        self.pool_xf_1 = nn.MaxPool1d(3)
        self.conv_xf_2 = nn.Conv1d(in_channels=n_filters, out_channels=n_filters * 2, kernel_size=8)
        self.pool_xf_2 = nn.MaxPool1d(3)
        self.conv_xf_3 = nn.Conv1d(in_channels=n_filters * 2, out_channels=n_filters * 4, kernel_size=8)
    ...
    def forward(self, data):
        ...
        # concat
        #xc = torch.cat((x, xt), 1)
        xc = torch.cat((drug, fingerprints,xt), 1)
