...
class TestbedDataset(InMemoryDataset):
    def __init__(self, root='/tmp', dataset='davis', 
                 xd=None,xds=None,xf=None, xt=None, y=None, transform=None,
        ...
        if os.path.isfile(self.processed_paths[0]):
            ...
            self.process(xd,xds, xf,xt, y,smile_graph)
            self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    def process(self, xd,xds,xf, xt, y,smile_graph):
        ...
        for i in range(data_len):
            ...
            GCNData.drug = torch.FloatTensor([drug])
            GCNData.fingerprints = torch.FloatTensor([fingerprints])
