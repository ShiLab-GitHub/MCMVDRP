...
def save_mix_drug_cell_matrix():
    f = open(folder + "PANCANCER_IC_urogenital_system.csv")
    reader = csv.reader(f)
    next(reader)
    ...
    for data in temp_data:
        ...
        if drug in drug_dict and cell in cell_dict:
            ...
            xf.append(smile_to_fingerprint(drug_smile[drug_dict[drug]]))
            xc.append(cell_feature[cell_dict[cell]])
            y.append(ic50)
            bExist[drug_dict[drug], cell_dict[cell]] = 1
            lst_drug.append(drug)
            lst_cell.append(cell)
            list_smile.append(drug_smile[drug_dict[drug]])
    with open('drug_dict', 'wb') as fp:
        pickle.dump(drug_dict, fp)

    XDS = [seq_cat_sml(t) for t in list_smile]

    xf,xd,xds, xc, y = np.asarray(xf),np.asarray(xd),np.asarray(XDS), np.asarray(xc), np.asarray(y)

    size = int(xd.shape[0] * 0.8)
    size1 = int(xd.shape[0] * 0.9)
    size2 = int(xd.shape[0] * 1)
    ...
    xd_test = xd[size1:size2]

    xds_train = xds[:size]
    xds_val = xds[size:size1]
    xds_test = xds[size1:size2]

    xc_train = xc[:size]
    xc_val = xc[size:size1]
    xc_test = xc[size1:size2]

    xf_train = xf[:size]
    xf_val = xf[size:size1]
    xf_test = xf[size1:size2]
    ...
    train_data = TestbedDataset(root='data', dataset=dataset+'_train_urogenital_system_mix', xd=xd_train,xds=xds_train,xf=xf_train,xt=xc_train, y=y_train, smile_graph=smile_graph)
    val_data = TestbedDataset(root='data', dataset=dataset+'_val_urogenital_system_mix', xd=xd_val, xds=xds_val,xf = xf_val,xt=xc_val, y=y_val, smile_graph=smile_graph)
    test_data = TestbedDataset(root='data', dataset=dataset+'_test_urogenital_system_mix', xd=xd_test,xds=xds_test,xf=xf_test, xt=xc_test, y=y_test, smile_graph=smile_graph)
...
def save_blind_drug_matrix():
    ...
    for drug,values in dict_drug_cell.items():
        ...
        for v in values:
            ...
            if pos < size:
                ...
                xf_train.append(smile_to_fingerprint(drug_smile[drug_dict[drug]]))
                list_smile_train.append(drug_smile[drug_dict[drug]])
                xc_train.append(cell_feature[cell_dict[cell]])
                y_train.append(ic50)
            elif pos < size1:
                xd_val.append(drug_smile[drug_dict[drug]])
                xf_val.append(smile_to_fingerprint(drug_smile[drug_dict[drug]]))
                list_smile_val.append(drug_smile[drug_dict[drug]])
                xc_val.append(cell_feature[cell_dict[cell]])
                y_val.append(ic50)
            else:
                xd_test.append(drug_smile[drug_dict[drug]])
                xf_test.append(smile_to_fingerprint(drug_smile[drug_dict[drug]]))
                list_smile_test.append(drug_smile[drug_dict[drug]])
    ...
    xds_train = [seq_cat_sml(t) for t in list_smile_train]
    xds_val = [seq_cat_sml(t) for t in list_smile_val]
    xds_test = [seq_cat_sml(t) for t in list_smile_test]
    print(len(y_train), len(y_val), len(y_test))

    xd_train,xf_train,xds_train, xc_train, y_train = np.asarray(xd_train),np.asarray(xf_train),np.asarray(xds_train), np.asarray(xc_train), np.asarray(y_train)
    xd_val,xf_val,xds_val, xc_val, y_val = np.asarray(xd_val),np.asarray(xf_val),np.asarray(xds_val), np.asarray(xc_val), np.asarray(y_val)
    xd_test,xf_test,xds_test, xc_test, y_test = np.asarray(xd_test),np.asarray(xf_test),np.asarray(xds_test), np.asarray(xc_test), np.asarray(y_test)

    dataset = 'GDSC'
    print('preparing ', dataset + '_train.pt in pytorch format!')
    train_data = TestbedDataset(root='data', dataset=dataset+'_train_blind', xd=xd_train,xds=xds_train,xf=xf_train, xt=xc_train, y=y_train, smile_graph=smile_graph)
    val_data = TestbedDataset(root='data', dataset=dataset+'_val_blind', xd=xd_val,xds=xds_val, xf=xf_val,xt=xc_val, y=y_val, smile_graph=smile_graph)
    test_data = TestbedDataset(root='data', dataset=dataset+'_test_blind', xd=xd_test,xds=xds_test,xf=xf_test, xt=xc_test, y=y_test, smile_graph=smile_graph)
...
def save_blind_cell_matrix():
    ...
    xd_train,xds_train,xf_train, xc_train, y_train = np.asarray(xd_train),np.asarray(xds_train),np.asarray(xf_train), np.asarray(xc_train), np.asarray(y_train)
    xd_val,xds_val,xf_val, xc_val, y_val = np.asarray(xd_val),np.asarray(xds_val), np.asarray(xf_val),np.asarray(xc_val), np.asarray(y_val)
    xd_test,xds_test, xf_test,xc_test, y_test = np.asarray(xd_test),np.asarray(xds_test),np.asarray(xf_test), np.asarray(xc_test), np.asarray(y_test)

    dataset = 'GDSC'
    print('preparing ', dataset + '_train.pt in pytorch format!')
    train_data = TestbedDataset(root='data', dataset=dataset+'_train_cell_blind', xd=xd_train,xds=xds_train,xf=xf_train, xt=xc_train, y=y_train, smile_graph=smile_graph)
    val_data = TestbedDataset(root='data', dataset=dataset+'_val_cell_blind', xd=xd_val,xds=xds_val, xf=xf_val,xt=xc_val, y=y_val, smile_graph=smile_graph)
    test_data = TestbedDataset(root='data', dataset=dataset+'_test_cell_blind', xd=xd_test,xds=xds_test,xf=xf_test, xt=xc_test, y=y_test, smile_graph=smile_graph)
